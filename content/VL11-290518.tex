...
&= \sum^m_{i=1} (\dv{t} a_i (t)) \big\vert_{t=0} w_i (0)\\
...
Sei $f$ glatt und $f(0)=p$. 
Außerdem ist
\begin{align}
& \dd f \big\vert_0 (\pdv{x}) = f_x (0) = u\\
& \dd f (\pdv{y}) = f_y(0) = v.
\end{align}
Es sei eine Familie von Kurven $c_s$ wobei $0 \leq s \leq 1$ und $c_s (0) = c_s(1) = 0$.
$P_s$ sei die Parallelverschiebung längs $c_s$.
Wir definieren
\begin{align}
c : I \times I &\to \mfk
(s, t) &\mapsto c(s,t),
...
1 & \mathrm{Index}(B) + 1 \leq i \leq n\\
\end{array}
\right. 
\end{align}
...
\begin{bem}
\begin{itemize}
\item Eine solche Metrik heißt Riemannsch, wenn $g_p$ positiv definit für alle $p$ in $\mfk$ ist.
\item Eine solche Metrik heißt Lorentz-Metrik, falls $\mathrm{Index} (g_p) = 1$ für alle $p$ in $\mfk$.
\end{itemize}
\end{bem}

\textbf{Zu Semi-Riemannsch}:
Index potentiall $\geq 1$, aber auch Riemannsche Metriken werden als Semi-Riemannsch bezeichnet.
